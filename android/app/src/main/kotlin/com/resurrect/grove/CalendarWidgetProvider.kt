...
class CalendarWidgetProvider : AppWidgetProvider() {
    ...
    private fun cycleHabit(flutterPrefs: SharedPreferences,
        ...
        if (habits.isEmpty()) return
            ...
            val idx       = habits.indexOf(currentId)
        ...
        private fun updateWidget(ctx: Context, mgr: AppWidgetManager, widgetId: Int) {
            ...
            val prevMonthCal    = if (month == 1) GregorianCalendar(year - 1, 11, 1)
            else            GregorianCalendar(year, month - 2, 1)
                ...
                for (cell in 0 until 42) {
                    ...
                    if (day < 1 || day > daysInMonth) {
                        val overflowDay = if (day < 1) daysInPrevMonth + day else day - daysInMonth
                        views.setTextViewText(id, "$overflowDay")
                        views.setInt(id, "setBackgroundResource", R.drawable.cal_cell_empty)
                        views.setInt(id, "setTextColor", 0x33E0EBE0.toInt())
                    } else {
